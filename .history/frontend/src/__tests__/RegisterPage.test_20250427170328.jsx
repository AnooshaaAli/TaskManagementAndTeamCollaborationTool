...
describe("LoginPage Component", () => {
  ...
  test('redirects to login on successful registration', async () => {
    ...
    await waitFor(() => {
      // Verify the redirect URL, ensuring assignMock is called with "/login"
      expect(assignMock).toHaveBeenCalledWith('/login'); 
    });

    // Check if the redirect happens after the setTimeout
    expect(assignMock).toHaveBeenCalledTimes(1); // Ensure it's called exactly once
  });  
