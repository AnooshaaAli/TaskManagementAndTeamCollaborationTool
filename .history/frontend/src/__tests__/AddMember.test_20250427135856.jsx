...
describe("TeamMemberForm Component", () => {
  ...
  test("renders input and add button", async () => {
    ...
    const htmlContent = document.body.innerHTML; // Log the HTML
    console.log(htmlContent); // You can inspect the console output to check if the input is there.
  ...
  test("successfully adds a member", async () => {
    ...
    require("../components/AddMember").addMemberToTeam.mockResolvedValue(mockResponse);
    ...
    await waitFor(() => {
      expect(require("../components/AddMember").addMemberToTeam).toHaveBeenCalledTimes(1);
