...
import TeamMemberForm from '../components/AddMember'; // Assuming the component and function are in the same file
import { addMemberToTeam } from '../components/AddMember'; // Importing the function for direct testing

// Mock the addMemberToTeam function
jest.mock('../components/AddMember', () => ({
    ...jest.requireActual('../components/AddMember'),  // Keep the original component intact
...
describe('addMemberToTeam function', () => {
    beforeEach(() => {
        fetch.mockClear(); 
        addMemberToTeam.mockClear(); // Clear mocks for the function before each test
    });
    ...
    it('should successfully add a member when response is OK and JSON', async () => {
        ...
        addMemberToTeam.mockResolvedValueOnce(mockResponse); // Mocking the function's resolved value
