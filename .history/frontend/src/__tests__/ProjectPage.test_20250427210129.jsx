...
describe('ProjectsPage', () => {
  ...
  test('navigates to dashboard on sidebar click', async () => {
    ...
    // Wait for the loading state to disappear, making sure the page has fully loaded
    await waitFor(() => screen.queryByText(/Loading your workspace.../i), { timeout: 5000 });
  
    // Query the sidebar navigation (which contains the "Dashboard" text)
    const sidebar = await screen.findByRole('navigation'); // Use findByRole to wait for it
    
    // Now find the "Dashboard" link within the sidebar
    const dashboardLink = within(sidebar).getByText(/Dashboard/i); // This ensures you're only looking within the sidebar
  
    // Simulate the click event on the "Dashboard" link
    fireEvent.click(dashboardLink);
  
    // Assert that the page navigates to the '/dashboard' route
