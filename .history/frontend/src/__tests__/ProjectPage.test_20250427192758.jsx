...
// Mocking fetch call for user data
...
describe('ProjectsPage', () => {
  ...
  test('renders user data after loading', async () => {
    ...
    // Wait for the username text to be rendered asynchronously
    const username = await screen.findByText('John Doe');
  
    // Assert that the username appears on the page
    expect(username).toBeInTheDocument();
  
    // Check if the user role is also rendered
    const userRole = await screen.findByText('Team Leader');
