...
describe("LoginPage Component", () => {
    ...
    test('renders all fields correctly', () => {
      ...
      test('redirects to login on successful registration (without actual redirect)', async () => {
        // Mock the window.location.href to avoid actual redirection
        ...
        await waitFor(() => {
          const successMessage = screen.getByText(/User registered successfully!/i);  // Flexible matching for the success message
          expect(successMessage).toBeInTheDocument();  // Ensure that the success message is rendered
        });
      
        // Check that the mock redirect function was called
        expect(assignMock).toHaveBeenCalledTimes(1);
        expect(assignMock).toHaveBeenCalledWith('/login'); // Ensure the redirect happens
      });
