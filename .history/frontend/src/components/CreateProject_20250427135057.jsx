...
// Mock the addMemberToTeam API call
jest.mock("../components/AddMember", () => ({
  __esModule: true,  // Ensure it's treated as ESModule
  default: jest.fn(),  // Mock the default export (TeamMemberForm)
  addMemberToTeam: jest.fn(),  // Mock the named export (addMemberToTeam)
}));
...
describe("TeamMemberForm Component", () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    localStorage.setItem("jwtToken", "mock-token");  // Prevent fetch from failing
  });
  ...
  test("successfully adds a member", async () => {
    ...
    await waitFor(() => {
      // Check that the mock function was called and that the success message is shown
