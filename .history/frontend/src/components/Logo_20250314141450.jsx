...
const Logo = ({ size = "md" }) => {
  ...
  return (
    ...
    >
      ...
      {/* Star near astronaut's helmet */}
      <polygon 
        points={`${astronautX - 30},${astronautY - 30} ${astronautX - 27},${astronautY - 22} ${astronautX - 35},${astronautY - 26} ${astronautX - 22},${astronautY - 26} ${astronautX - 30},${astronautY - 22}`} 
        fill="white" 
        opacity="0.9"
      />
      ...
      <polygon 
        points={`${astronautX + 10},${astronautY + 14} ${astronautX + 13},${astronautY + 24} ${astronautX + 23},${astronautY + 24} ${astronautX + 16},${astronautY + 30} ${astronautX + 18},${astronautY + 40} ${astronautX + 10},${astronautY + 34} ${astronautX + 2},${astronautY + 40} ${astronautX + 4},${astronautY + 30} ${astronautX - 4},${astronautY + 24} ${astronautX + 7},${astronautY + 24}`} 
        fill="white" 
        opacity="0.9"
      />
      ...
      {/* Additional stars for decoration */}
      {[...Array(4)].map((_, i) => {
        const starX = Math.random() * screenWidth;
        const starY = Math.random() * 180;
        return (
          <polygon
            key={i}
            points={`${starX},${starY} ${starX + 3},${starY + 8} ${starX - 5},${starY + 4} ${starX + 8},${starY + 4} ${starX},${starY + 8}`}
            fill="white"
            opacity="0.8"
          />
        );
