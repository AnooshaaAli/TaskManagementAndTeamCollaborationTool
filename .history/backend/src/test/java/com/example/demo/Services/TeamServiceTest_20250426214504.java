...
class TeamServiceTest {
    ...
    void testRemoveMemberFromProject_Success() {
        ...
        TeamHasMember teamHasMember1 = new TeamHasMember();
        teamHasMember1.setUser(user);
        teamHasMember1.setTeam(team);
    
        TeamHasMember teamHasMember2 = new TeamHasMember();
        teamHasMember2.setUser(currentUser);
        teamHasMember2.setTeam(team);
        
        // Mock repository responses
        when(userRepository.findById(1)).thenReturn(Optional.of(currentUser));  // Current user
        when(userRepository.findById(2)).thenReturn(Optional.of(user));  // User to be removed
        when(projectRepository.findById(1)).thenReturn(Optional.of(project));  // Project
        when(teamRepository.findByProject(any())).thenReturn(team);  // Team
        when(teamHasMemberRepository.findByTeamAndUser(any(), any())).thenReturn(Optional.of(teamHasMember1));  // Check if user is in the team
        
        // Perform the test method
        ResponseEntity<Map<String, String>> response = teamService.removeMemberFromProject("testuser", 1);
        
        // Assert the correct success response
