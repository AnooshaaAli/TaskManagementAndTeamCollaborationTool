...
public class AuthController {
    ...
    public ResponseEntity<?> getUserById(@PathVariable Integer userId, @RequestHeader("Authorization") String token) {
        ...
        try {
            ...
            // Fetch account details based on the extracted email (check if user is authenticated)
            Optional<Account> account = accountService.getAccountByEmail(email);
            ...
            if (account.isPresent()) {
                ...
                if (user.isPresent()) {
                    // Returning the requested user's data, since comments can be made by anyone
