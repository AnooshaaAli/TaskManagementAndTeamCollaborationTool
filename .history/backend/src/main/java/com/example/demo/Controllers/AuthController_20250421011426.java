...
public ResponseEntity<?> getUserById(@PathVariable Long userId, @RequestHeader("Authorization") String token) {
    ...
    try {
        // Validate token and extract user email from it
        String email = jwtUtil.extractEmail(token.replace("Bearer ", ""));
        Optional<Account> account = accountService.getAccountByEmail(email);

        // Check if the user is authorized to access the requested user details
        if (account.isPresent()) {
            Account user = account.get();

            // Ensure the logged-in user is fetching their own details or an admin is fetching another user's details
            if (user.getId().equals(userId)) {
                ...
                // Unauthorized if trying to access another user's details
