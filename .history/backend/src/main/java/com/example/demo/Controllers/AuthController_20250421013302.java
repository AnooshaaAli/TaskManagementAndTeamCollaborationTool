...
public class AuthController {
    ...
    public ResponseEntity<?> getUserById(@PathVariable Integer userId, @RequestHeader("Authorization") String token) {
        ...
        try {
            // Extract email from the JWT token to validate authentication
            String email = jwtUtil.extractEmail(token.replace("Bearer ", ""));
    
            // Fetch the account using the email from the token (just to validate the requester)
            Optional<Account> account = accountService.getAccountByEmail(email);
    
            // If the user is authenticated (account exists)
            if (account.isPresent()) {
                // Fetch the user based on the userId from the path
                Optional<User> user = userService.getUserById(userId);
    
                if (user.isPresent()) {
                    // Prepare response with the user data
                    response.put("userID", user.getAccount().getId());
                    response.put("username", user.getAc.getUsername());
